	...
	function calendarWidget(el, params) { 
		...
		var table = '';
            ...
            for (j=0;j<42;j++){
              ...
              if ((j<firstDay)){
                  table += ('<td class="other-month"><span class="day">' + '<a href=' + "'" + eventURL + "'" + '>' +  (prev_days - firstDay + j + 1) + '</a>' + '</span></td>');
			  } else if ((j>=firstDay+getDaysInMonth(month,year))) {
				i = i+1;
				table += ('<td class="other-month"><span class="day">'+ '<a href=' +"'"+ eventURL +"'"+ '>' + i + '</a>' +'</span></td>');			 
              }else{
			      table += ('<td class="current-month-day' + (j - firstDay + 1) + '"><span class="day">' + '<a href=' + "'" + eventURL + "'" + '>' + (j - firstDay + 1) + '</a>' + '</span></td>');
              }
              if (j%7==6)  table += ('</tr>');
            }
